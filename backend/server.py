...
# ---------- Pengaturan (Data LIS mentah disimpan permanen) ----------
@api.get('/settings')
async def get_settings(user: dict = Depends(auth.get_current_user)):
    """Data LIS mentah (Data LIS Mentah) disimpan permanen di MongoDB, tidak pernah dihapus otomatis,
    karena kolom harian 1-31 Pemantauan Stok dihitung langsung dari data ini setiap kali diakses."""
    last_cleanup = await import_log_col.find_one(
        {'type': 'lis_cleanup'}, {'_id': 0}, sort=[('created_at', -1)])
    return {'auto_delete_enabled': False, 'last_cleanup': last_cleanup}
...
# ---------- Data LIS mentah: disimpan permanen (tidak ada auto-hapus) ----------
async def _backfill_lis_raw_uploaded_at():
    """Beri timestamp 'uploaded_at' = sekarang ke data lama yang belum punya (untuk histori/urutan saja)."""
